...
<div class="bg-gray-100 min-h-screen"> 
     <main class="container mx-auto px-8 p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        <div v-for="restaurant in restaurants" :key="restaurant.id"  data-aos="zoom-in-down"  class="bg-white  rounded-lg shadow-xl m-4 p-2 h-30 w-60 overflow-hidden transition-transform transform hover:scale-105">
          <div class=" flex flex-col items-center">
            <img class="  w-40 h-32 object-cover rounded-lg" :src= "restaurant.logo"  alt="restaurant logo" />
            <h2 class="font-bold text-xl text-orange-700 uppercase">{{ restaurant.name }}</h2>
            ...
            <RouterLink :to="{ name: 'Menu', params: { id_restaurant:restaurant.id_restaurant}}"> <button  class="mt-4 inline-block bg-orange-400 text-white rounded-xl py-2 px-4 hover:bg-orange-700 transition">See the Menu</button></RouterLink>
          </div>
        </div>
      </div>
     </main>
